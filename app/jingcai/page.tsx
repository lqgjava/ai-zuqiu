...
function bestResult(match: { aiPrediction: { home: number; draw: number; away: number } }, strings: any) {
  if (match.aiPrediction.home > match.aiPrediction.away && match.aiPrediction.home > match.aiPrediction.draw) {
    return strings.jingcai.statusHighRisk ? strings.jingcai.statusHighRisk : 'Home win';
  }
...
export default function JingcaiPage() {
  ...
  const { matches, loading, error, source } = useJingcaiData(jingcaiMatches);
    ...
    <main className="main-container py-12">
      ...
      <section className="mb-12 space-y-6">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <h1 className="mt-4 text-4xl font-bold text-primary sm:text-5xl">{strings.jingcai.title}</h1>
            <p className="mt-2 max-w-2xl text-muted">{strings.jingcai.subtitle}</p>
          </div>
        ...
        {loading ? <p className="text-sm text-muted">{strings.jingcai.refreshStatus(true)}</p> : null}
      </section>
      ...
      <section className="grid gap-6 lg:grid-cols-[1.6fr_1fr] mb-12">
        <div className="data-block-primary rounded-lg p-6">
          ...
          <h2 className="mt-3 text-2xl font-bold text-primary">{strings.jingcai.topicTitle}</h2>
          <p className="mt-3 text-muted">{strings.jingcai.topicText}</p>
      ...
      <section className="grid gap-6 lg:grid-cols-[1.1fr_0.9fr_0.9fr] mb-12">
        ...
        <Card className="space-y-6 rounded-lg">
          <div>
            ...
            <h2 className="mt-2 text-2xl font-bold text-primary">{strings.jingcai.matchListTitle}</h2>
          </div>
            ...
            {matches.slice(0, 8).map((match) => (
              <div key={match.id} className="match-card">
                ...
                <div className="pt-3 border-t border-neutral-dark">
                  <p className="text-sm text-muted mb-2">{match.stage} · {match.status}</p>
        ...
        <Card className="space-y-6 rounded-lg">
          <div>
            ...
            <h2 className="mt-2 text-2xl font-bold text-primary">{strings.jingcai.dataMenuSubtitle}</h2>
            <p className="mt-2 text-sm text-muted">{strings.jingcai.subtitle}</p>
          </div>
          ...
          <div className="space-y-3">
            {matches.slice(0, 3).map((match) => (
              <div key={`${match.id}-menu`} className="card rounded-lg p-4">
                ...
                <p className="mt-2 text-sm text-muted">{match.homeScore !== undefined && match.awayScore !== undefined ? `Score: ${match.homeScore}-${match.awayScore}` : 'Pending'}</p>
        ...
        <Card className="space-y-6 rounded-lg">
          <div>
            ...
            <h2 className="mt-2 text-xl font-bold text-primary">{strings.jingcai.aiRecommendationTitle}</h2>
          </div>
          ...
          <div className="space-y-3">
            {matches.slice(0, 5).map((match) => (
              <div key={match.id} className="prediction-block">
                ...
                <p className="mt-1 text-xs text-muted">{match.aiPrediction.summary}</p>
              </div>
            ))}
          </div>
        </Card>
      ...
      <section className="grid gap-6 lg:grid-cols-3 mb-12">
        <Card className="rounded-lg p-6">
          <p className="section-label">{strings.jingcai.matchListLabel}</p>
          <h3 className="mt-3 text-2xl font-bold text-primary">{strings.jingcai.matchListTitle}</h3>
          <p className="mt-3 text-muted">{strings.jingcai.subtitle}</p>
        </Card>

        <Card className="rounded-lg p-6">
          <p className="section-label">{strings.jingcai.coreMatchLabel}</p>
          <h3 className="mt-3 text-xl font-bold text-primary">{strings.jingcai.coreMatchLabel}</h3>
        ...
        <Card className="rounded-lg p-6">
          ...
          <p className="mt-3 text-muted text-sm">{strings.jingcai.riskAlertText}</p>
